...
// a one-click path to migrate their Supabase row data into R2 blobs (Phase 1
// of the R2 cloud-sync migration). See openspec spec "Banner-prompted
// client-side migration for existing M4-era users" for full requirements.
//
// Phase 1 scope: detects M1 bundle only. Phase 2 (task 4.5) extends detection
// to M2 + bookmarks.

import { useCallback, useEffect, useState } from 'react'
import type { SupabaseClient } from '@supabase/supabase-js'
import { detectM1NeedsMigration, migrateM1FromSupabase } from '../lib/sync/r2/migrate-from-supabase'
...
export function MigrationBanner({ supabase, userId }: Props) {
  ...
  const recheck = useCallback(async () => {
    ...
    try {
      const probe = await detectM1NeedsMigration(supabase, userId)
  ...
  const handleMigrate = useCallback(async () => {
    ...
    setErrorMsg(null)
    try {
      const result = await migrateM1FromSupabase(supabase, userId)
      if (result.status === 'uploaded' || result.status === 'already-present' || result.status === 'no-rows') {
        // Migration succeeded — drop dismiss log so a future re-prompt (Phase 2
        // bundles) starts clean.
        clearDismissLog()
        setVisibility('done')
        // Auto-hide after 5s so the user gets confirmation but the banner clears.
        setTimeout(() => setVisibility('hidden'), 5000)
      } else {
        setVisibility('error')
        setErrorMsg(result.error ?? '遷移失敗，請稍後重試')
      }
    } catch (err) {
      setVisibility('error')
