...
/**
 * Lock the tier-promote core safety (add-neurons-dupe-fusion): last-copy
 * protection, surplus accounting, P0 guard, and the consume-K → mint-T−1 flow.
 */
...
describe('tier-promote eligibility', () => {
  it('protects the sole individual of a slot (last-copy)', async () => {
    await db.neuronInstances.bulkPut([
      seed(FAMILY.p4[0], 'P4'), // slot A: 1 only → protected
      seed(FAMILY.p4[1], 'P4'), // slot B: 3 → 2 surplus
      seed(FAMILY.p4[1], 'P4'),
      seed(FAMILY.p4[1], 'P4'),
    ])
  ...
  it('disabled below the K threshold', async () => {
...
describe('tier-promote execution', () => {
  it('consumes K surplus P4 individuals and mints a P3 individual', async () => {
    // slot A: K instances (K-1 surplus) + slot B: 2 instances (1 surplus) → ≥ K surplus
