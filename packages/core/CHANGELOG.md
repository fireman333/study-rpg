...
- `ExplanationTableImage` type (`{ src; caption? }`) and an optional
  `Question.explanationTableImages?: ExplanationTableImage[]` field — the faithful image-crop tier
  for 詳解 tables that cannot be reconstructed as structured text without medical guessing
  (scrambled-cell / embedded-figure tables). Rendered AFTER the explanation; additive and optional
  (`explanation` / `id` / `answer` unchanged; no behavior change for existing consumers). Pre-1.0
  PATCH bump per the `core-npm-package` additive policy.
