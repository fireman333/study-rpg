...
#### Scenario: Desktop renders the family grid + maze as a two-column master-detail
- **WHEN** the homepage renders at ≥ 768px and the maze is expanded
- **THEN** the subject cards occupy the left column and the single maze instance occupies a sticky right detail panel
- **AND** selecting a subject card focuses the right-panel maze on that family's cluster

#### Scenario: Mobile expands the maze adjacent to the selected card
- **WHEN** the homepage renders at < 768px
- **THEN** the subject cards stack and the single maze panel expands adjacent to the selected card (not a second canvas), focused on that family
- **AND** there is exactly one `MazeGrid` instance (no per-card canvas)
