...
- **Downgrade gate**: after seven consecutive days with `cpuTimeP99 < 10 ms` on every `scriptName`/`datetimeMinute` bucket of `study-rpg-sync-worker`, the owner downgrades the account to Workers Free in the dashboard. The downgrade itself is an owner action outside this change.
