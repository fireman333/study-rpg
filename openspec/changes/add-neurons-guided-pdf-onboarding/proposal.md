...
- **Auth onboarding banner (any local-PDF-capable platform)**: a global, dismissible onboarding card shown when the platform can open a local PDF (Tauri desktop, OR a Chromium-desktop web browser with File System Access — `isLocalPdfSupported()`) AND no folder is granted (teaches 1. download 2. grant; dismisses after grant), plus the inline missing-file prompt above. Hidden where local-PDF is unsupported (Safari / Firefox / mobile), where granting a folder would be a dead end.
