...
- [x] 7.3 Desktop end-to-end (**OWNER, verified 2026-06-27**): rename a downloaded booklet to a random name → still resolves by fingerprint; missing booklet → guided prompt + official link opens + rescan resolves; onboarding banner shows pre-grant / hides post-grant; desktop bug-report category appears + platform context attaches. (Done on the `pnpm dev:tauri` WKWebView build after the 7.6 blank-render fix landed.)
- [x] 7.4 Fingerprint validation (**OWNER PDFs, verified 2026-06-27**): all 44 booklets resolve; 113-2醫學二 is the size-only low-confidence one (confirm-note path verified)
...
- [x] 7.7 **Onboarding banner generalized to web** (owner request): the first-run banner was desktop-only (`isDesktop()`); generalized its gate to `isLocalPdfSupported()` so it ALSO shows on Chromium-desktop web (Chrome/Edge with File System Access) — where the FSA grant + 46 official-Drive download links already work — and stays hidden on Safari/Firefox/mobile (no FSA → granting a folder is a dead end). Renamed `DesktopPdfOnboardingBanner` → `PdfOnboardingBanner` + platform-neutral copy (dropped「（桌面版）」). App-global mount unchanged (so it appears on the homepage). Spec delta (`neurons-tauri-shell` banner Requirement) + proposal updated. **Web (Blink) verified** via Chrome MCP: banner renders on FSA-Chrome, 展開 → 46 booklet links load with real `drive.google.com` URLs, grant-folder button present, dismiss persists + hides; 712 vitest + typecheck green.
