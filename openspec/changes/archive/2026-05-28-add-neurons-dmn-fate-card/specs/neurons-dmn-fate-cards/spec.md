...
A new service `apps/neurons-tw/src/services/dmn-trigger.ts` SHALL be initialized at app boot via the app's main entry point. The service SHALL:

- Register listeners on the event bus for `streak.dayIncreased`, `connectome.variantSlotUnlocked`, and `connectome.actionPotentialThresholdCrossed`
...
- Persist all state via Dexie writes wrapped in transactions, emitting `dmn.drawsGranted` events only after commit
...
- **THEN** event bus listeners SHALL be registered exactly once (singleton; second call is no-op)
...
- New table `dmnEventLog`:
  - Primary key: `cardId`
  - Other columns: `dispatchedAt`, `deviceId`
- New table `dmnActiveBuffs`:
  - Primary key: auto-incremented `id`
  - Indexed columns: `expiresAt`, `buffKind`
  - Other columns: `familyId` (nullable, used by `family-buff` only), `payload` (JSON blob for type-specific data)
...
### Requirement: R2 bundle SHALL include four optional dmn-* fields participating in cross-device sync

The neurons R2 bundle (`users/<user_id>/neurons-snapshot.json.gz`) SHALL include four new optional top-level fields:

- `dmnCards?: DmnCardRow[]` — full per-row sync, LWW on `obtainedAt`
- `dmnMeta?: { dailyDrawsConsumed: number, dailyResetDate: string, drawsAvailable: number, timeAxisMinutesAccrued: number, timeAxisDrawsConsumedToday: number, behaviorAxisDrawsConsumedToday: number }` — single object, LWW per field on `updatedAt`
- `dmnEventLog?: DmnEventLogRow[]` — **monotonic-union merge** (not LWW; see event-log requirement)
- `dmnActiveBuffs?: DmnActiveBuffRow[]` — LWW on `expiresAt`; expired buffs (`expiresAt < now`) SHALL be filtered out client-side after merge

All four fields SHALL be optional (`undefined` permitted). A v1 client reading a v2 bundle SHALL silently drop these fields (see modified neurons-deploy requirement). A v2 client reading a v1 bundle SHALL preserve-on-omission: missing fields SHALL be treated as empty / default.

#### Scenario: Bundle round-trip preserves dmn-* fields across v2 clients

- **GIVEN** device A (v2 client) writes a bundle with `dmnCards = [card-1]`, `dmnEventLog = [{cardId: 'card-1', dispatchedAt: '...'}]`
- **WHEN** device B (v2 client) pulls the bundle
- **THEN** device B SHALL deserialize and apply both fields
- **AND** device B's local `dmnCards` SHALL contain `card-1`
