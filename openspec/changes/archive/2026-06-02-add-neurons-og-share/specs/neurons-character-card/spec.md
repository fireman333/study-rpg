...
### Requirement: A single share entry SHALL render a character card to a shareable PNG, entirely client-side

The neurons mode SHALL provide one share entry that opens a modal previewing a rendered **character card** and offering export. The card SHALL be composed in-browser via the Canvas 2D API and exported as a PNG by `canvas.toBlob` → file download, with an optional `navigator.share({ files })` path on devices that support it. No server SHALL be contacted to build, render, or host the card.

#### Scenario: Player opens the share entry and previews the card
- **WHEN** the player activates the share entry
- **THEN** a modal SHALL open and render the character card onto a preview canvas
- **AND** a download action SHALL be available

#### Scenario: Download produces a PNG locally
- **GIVEN** the card preview has rendered
