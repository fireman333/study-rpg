...
#### Scenario: Desktop overview stacks the maze above the family grid
- **WHEN** the homepage renders at ≥ 768px, the maze is expanded, and no family is selected (`selectedFamilyId === null`)
- **THEN** the single maze instance renders as a full-width panel stacked ABOVE the subject-card grid and BELOW the exam-year filter chips (the same vertical order as < 768px)
...
- **AND** 🔭 全覽 exits detail mode back to the stacked overview (no separate 返回 button)
