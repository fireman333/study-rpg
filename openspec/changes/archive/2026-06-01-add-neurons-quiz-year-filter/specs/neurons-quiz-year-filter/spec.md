...
When the family × year selection yields zero questions, launching SHALL be allowed and `QuizModal` SHALL render a plain text empty state (e.g. 「所選年份下這科沒題目」) with a close affordance, distinct from the normal 「答完」 completion state. No quick-action button is required.
