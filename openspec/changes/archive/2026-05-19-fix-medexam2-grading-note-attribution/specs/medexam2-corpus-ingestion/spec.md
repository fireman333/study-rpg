...
- Extract the directive from any option matching the regex `/\s*(※\s*第\s*\d+\s*題[^。]*給分。?)\s*$/u` (anchored at end-of-option)
- Strip the matched directive from the option text
...
Option strings SHALL NOT contain the「※」grading-note marker. Explanations SHALL NOT contain the「※」marker outside the prepended blockquote header (any LLM echo of the directive inside `**X. ...**` bold blocks SHALL also be stripped — see Explanation merge requirement).
...
**Grading-note echo strip**: Any LLM echo of the「※第N題...給分。」directive that appears inside an explanation `**X. ...**` bold block SHALL also be stripped from that location (a separate Pass 0 in the sanitization pipeline targets the directive precisely — strip the directive only, not everything through closing `**`). The directive content is surfaced once at the top of the explanation via the blockquote prepended by `buildQuestion` (see Question-parser requirement); duplicate echoes inside per-option bold headings are visual noise and SHALL be removed.
