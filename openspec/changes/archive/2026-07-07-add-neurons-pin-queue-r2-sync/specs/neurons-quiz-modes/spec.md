...
### Requirement: questionFlags SHALL carry a synced nullable pinnedAt pin field

The `questionFlags` row SHALL carry a **nullable `pinnedAt?: number`** field that persists the 「置頂下次出征」pin (per `neurons-simplified-explanations`) as durable cross-device state. Effective pin = `pinnedAt != null`. Enqueue SHALL set `pinnedAt` to the current time with a fresh `updatedAt`; dequeue SHALL set `pinnedAt = null` with a fresh `updatedAt`. FIFO ordering SHALL be derived at read time by sorting pinned rows by `pinnedAt` ascending; `pinnedAt` SHALL NOT be added to the Dexie `.stores()` index list (it is a non-indexed field, so no Dexie `.version()` bump and no upgrade fixture are required). Every writer of a `questionFlags` row SHALL preserve `pinnedAt` it does not own: the four boolean-flag setters (✨/🤔/👁/💡) SHALL carry the existing `pinnedAt` through when they persist, and SHALL NOT drop a pin. The R2 `questionFlags` sync adapter SHALL serialize `pinnedAt` and, on apply, SHALL treat an **omitted** `pinnedAt` key as preserve-on-omission (keep the locally-set value) while treating an **explicit `null`** as a dequeue that clears the pin under the existing per-row LWW on `updatedAt` (a newer row whose `pinnedAt` is null beats an older row that still holds a pin, so the removal propagates cross-device with no tombstone). Adding the synced `pinnedAt` field SHALL bump the R2 bundle `SCHEMA_VERSION` from 24 to 25 with reader tolerance (an older client reading a v25 bundle drops the unknown `pinnedAt`; a newer client reading a v24 bundle reads `pinnedAt` as absent = not pinned, so a pin never resurrects).

#### Scenario: Enqueue sets a pin timestamp

- **WHEN** the player pins a question via「置頂下次出征」
- **THEN** that question's `questionFlags.pinnedAt` SHALL be set to the current time with a fresh `updatedAt`
- **AND** the row's four boolean flags SHALL be preserved unchanged

#### Scenario: A boolean-flag setter preserves an existing pin

- **GIVEN** a question whose `questionFlags.pinnedAt` is set (pinned)
- **WHEN** the player later toggles ✨「太簡單」on that question (via the existing `setEasy` / `toggleEasy` path)
- **THEN** `easyMarked` SHALL update
- **AND** `pinnedAt` SHALL remain set (the ✨ setter SHALL NOT reconstruct the row without `pinnedAt`)

#### Scenario: R2 adapter round-trips pinnedAt and clears it on explicit null

- **GIVEN** a local `questionFlags` row with `pinnedAt` set
- **WHEN** the row is serialized and re-applied through the `questionFlags` R2 adapter
- **THEN** `pinnedAt` SHALL survive the round-trip
- **AND** an incoming row with an **explicit** `pinnedAt: null` and a newer `updatedAt` SHALL clear the local pin (dequeue propagates under per-row LWW, no tombstone)
- **AND** an incoming row that **omits** `pinnedAt` SHALL NOT clear the locally-set pin (preserve-on-omission)

#### Scenario: Older client benignly ignores the pin field

- **GIVEN** a client built before this change (unaware of `pinnedAt`)
- **WHEN** it reads a v25 bundle carrying a row with `pinnedAt` set
- **THEN** it SHALL drop the unknown `pinnedAt` field (existing reader tolerance) and neither crash nor act on the pin
- **AND** when it later writes that flag row it SHALL omit `pinnedAt`, which a newer client SHALL treat as preserve-on-omission (the pin does not resurrect and is not wiped by the older client)
...
Because the `questionFlags` row now carries four coexisting boolean flags (`easyMarked`, `guessedMarked`, `wrongAnswerMarked`, `insightMarked`) **plus a nullable synced `pinnedAt` pin field** (per the `questionFlags SHALL carry a synced nullable pinnedAt pin field` requirement), every existing and new writer SHALL preserve the fields it does not own: the existing ✨/🤔 setters/toggles SHALL NOT clear the two error-cause flags or the pin when they persist, and the new error-cause setters SHALL NOT clear ✨/🤔 or the pin. The R2 `questionFlags` sync adapter SHALL serialize and apply the four boolean fields, and on apply SHALL preserve any locally-set flag that an incoming row omits (preserve-on-omission). Serializing the four boolean fields required no R2 SCHEMA_VERSION bump; adding the synced `pinnedAt` field DOES bump the R2 bundle `SCHEMA_VERSION` from 24 to 25 (see the `pinnedAt` pin-field requirement for the bump and reader-tolerance contract).
...
- **THEN** the write SHALL use the existing store and primary key with no Dexie `.stores()` change (the boolean fields themselves needed no R2 SCHEMA_VERSION bump)
...
- **AND** an incoming row that omits `insightMarked` SHALL NOT clear the locally-set value (preserve-on-omission)
