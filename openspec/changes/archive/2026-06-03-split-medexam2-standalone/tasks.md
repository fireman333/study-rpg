...
- [x] 5.4 Local structural verify (ROUTES drives both copy + `_redirects`; `node --check` passes; no `medexam2`/`/2nd/` left in active CF config — only explanatory comments) **+ post-merge CI green confirmed** (2026-06-03, commit `bcb5638`): `gh run list --branch main` shows both **Deploy Cloudflare Pages** (1m3s ✓) and **Deploy to GitHub Pages** (51s ✓) success.
...
- [x] 6.2 **Verified in a real browser (Chrome MCP, 2026-06-03 post-merge)**: `https://fireman333.github.io/study-rpg/hospital/` → `https://med-study-rpg.com/2nd/` (title 「二階國考經營 RPG」, app mounted); sub-route `…/hospital/bookmarks` → `https://med-study-rpg.com/2nd/#/bookmarks` (HashRouter route preserved). GH `/hospital/` now serves 404.html (404 status, SPA-fallback) containing the new redirect target ×2 and zero old `/hospital/#/` logic — no 二階 bundle served there anymore.
...
- [x] 7.3 Invariant check **complete (post-merge prod smoke, 2026-06-03)**: backend untouched (no sync Worker / R2 / D1 / KV / Supabase Auth file in this change's diff); `med-study-rpg.com/2nd/` → 200 `x-served-by=edge-router-2nd` (二階 still served by the standalone project via edge-router, unaffected by the combined redeploy); `/1st/` → 200 + `/neurons/` → 200 (combined project unaffected); saves seamless + `/2nd/` URL unchanged (prod §4.3 + this smoke); old GH `/hospital/` → 301 confirmed (§6.2). Rollback path documented (`CLAUDE.md` subsection + design Migration Plan, 2-phase, reversible).
