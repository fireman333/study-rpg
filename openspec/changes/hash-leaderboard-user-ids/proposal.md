...
- **Worker 先、前端後**：新 var `LEADERBOARD_RAW_ID_COMPAT = "1"` 期間公開面同時帶舊欄位（`user_id`、原始 `authorKey`），讓舊 bundle 仍認得自己；兩個前端上線後刪掉這個 var 再 deploy 一次。留言板 edge cache key 改成 `v2-keyed` / `v2-compat`，deploy 當下不會吐出舊形狀的快取。
...
- **Owner 關卡（agent 不執行）**：`wrangler secret put LEADERBOARD_PLAYER_KEY_SECRET` → Worker deploy（compat on）→ `npm publish` core 0.7.0 → 二階 bump + deploy、neurons deploy（= push main）→ 刪 compat var → Worker deploy。順序與 rollback 見 design D7。
