...
| **M4.5 — In-app bug report → Supabase** | `bug_reports` table (migration 0004) + RLS + per-app `BugReportModal` (一階 SettingsPanel 新 section / 二階 HelpMenu 9th accordion) + auto-context snapshot (game_state JSONB / route / commit_sha / recent_console_errors ring buffer) + force sign-in gate + per-field opt-out. Owner reads via dashboard SQL today; future `/bug-reports` skill follows. 詳見 `openspec/changes/add-bug-report-pipeline/` + `docs/BUG_REPORTING.md`. | 🚧 in change |
