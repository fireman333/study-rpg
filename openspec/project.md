...
| **二階 standalone split (`split-medexam2-standalone`)** | 把 二階 (`apps/medexam2-hospital-tw` + `theme-pixel-hospital` + `content-medexam2-tw`) 抽成獨立 repo (`fireman333/study-rpg-2nd`，`~/coding-scratch/study-rpg-2nd`)，consume `@study-rpg/core` from npm `^0.6.0`（非 workspace symlink，core fix 走 publish→bump 傳遞）。獨立 CF Pages project `med-study-rpg-2nd` + edge-router Worker 綁 `med-study-rpg.com/2nd/*` reverse-proxy，玩家 URL 不變；backend（sync Worker `api.med-study-rpg.com` / R2 / D1 / Supabase Auth / `user_id`）完全不動，存檔無縫（§4.3 prod 已驗）。原 monorepo combined pipeline 拿掉 二階（§5：`build-cf-pages-dist.mjs` ROUTES / `build:cf` / `deploy-cf-pages.yml` 二階 build / `deploy.yml` 二階 build+merge），GH `/study-rpg/hospital/` → 301 redirect 到 `/2nd/`（§6，via `apps/medexam-tw/public/404.html`）。未來 二階 feature 在新 repo 開發，不回此 monorepo。詳見 `openspec/changes/split-medexam2-standalone/`. | 🔄 §0–§6 + §7.1/7.2 done (2026-06-03)；剩 merge `track-m2` → `main`（merge gate）+ §6.2/§7.3 prod verify + archive |
...
| **M_3rd ext — 出征完成獎勵 (add-neurons-expedition-rewards)** | Phase 4：把 DMN 抽卡「時間軸」由閱讀分鐘改綁**出征清除錯題**（`onExpeditionComplete` no-op → 發放）。Per-session 百分比門檻 + clamp：每場出征清掉 `clamp(round(pct×開場錯題數),min,max)` 題各 +1 draw，milestone 表 `[{25%,3–15},{50%,6–30}]`（每日上限 2 = milestone 數）。閱讀解除 DMN 耦合（仍餵 maze 能量 + 累積閱讀），行為軸不變 → DMN 每日總抽 ≤ 5。**純 meta key-value：不 bump Dexie、刻意凍結 synced key 名稱（`dmnTimeAxisMinutesAccrued` 改裝清除數）→ 不動 SYNCED_META_KEYS、R2 SCHEMA_VERSION 維持 12**。指標 = wrong→correct flip（pool 只含錯題天生防刷 + cap 雙重防刷）。改 dmn-trigger（`creditExpeditionDraws`）/ reading-timer / expedition / dmn-types + UI 文案（DmnDrawButton/ProgressRing/HelpMenu/HomepageOnboarding）。spec deltas：`neurons-dmn-fate-cards` + `neurons-study-squad` + `neurons-homepage`（ring 改 expedition-axis）。261 Vitest 全綠 + typecheck clean。詳見 `openspec/changes/add-neurons-expedition-rewards/`. | 🔄 code-complete (2026-06-04)；剩 verify + archive |
| **M_3rd ext — DMN 獎勵對齊 maze (realign-dmn-event-rewards-to-maze)** | Follow-up to expedition-rewards：審視 5 個 DMN 事件獎勵 vs maze-promotion 後經濟，realign 2 個失準者。**family-buff**：原「+2 AP/正確」（AP 已不再 gate 進度、淪純分數）→ 改乘被 buff family 的 post-commit maze 能量 faucet ×`FAMILY_BUFF_ENERGY_MULT`（=2，dogfood-tunable）、1hr，不再碰 AP（`getActiveFamilyBuffBonus`→`getActiveFamilyBuffMultiplier`，套在 connectome 能量 faucet、與 mastery×streak 複合）。**quick-review-batch**：原 placeholder toast（neurons 無 SRS、且與出征重疊）→ 可點擊「▶ 5 題快速複習」CTA，開 ≤5 題錯題出征 mini-batch（`buildQuickReviewPool`），清除走同一條 `onExpeditionComplete → creditExpeditionDraws` → 閉環。順手修正 spec drift（variant-rate-up 文字對齊 roll-twice-take-rarer、streak-shield 對齊 correct-answer streak；行為不變）。**零 schema/sync 改動**（family-buff 仍走既有 `dmnActiveBuffs` table、能量是既有 per-branch meta；不 bump Dexie/R2/SYNCED_META_KEYS）。spec delta：`neurons-dmn-fate-cards`「Five DMN event types」MODIFIED。270 Vitest 全綠（+9）+ typecheck clean + Chrome MCP boot smoke。詳見 `openspec/changes/realign-dmn-event-rewards-to-maze/`. | 🔄 code-complete (2026-06-04)；剩 archive + commit（與 expedition 一起 merge） |
| **M_3rd ext — 單科考前救急 (add-neurons-single-subject-rescue)** | 鎖一科 + 綁考試日 + 每日分鐘預算 → 每天排「救分 ROI」佇列（`priority = Yield × Movability × Confidence × typeCoefficient ÷ EstTime`，含分診丟棄已會 / 沒救低頻 + 止損干預切換）。**Pre-reveal 雙鍵信心捕捉**（選項→「確定・有把握 / 確定・猜的」→ reveal，×1.5 hypercorrection 唯一住所）。D-scaled 診斷突襲 + concept 紅/黃/灰**三段式戰情圖**（count badge + `+N` overflow）+ RescueScore 質性回報（夯/普通/低迷，無假分數）+ 考試日早晨速掃。全畫面 overlay `RescueScene.tsx`（portal to body）；答題複用 QuizModal `rescueSubmit` 模式（走 `recordQuestionResult`+SRS，**刻意跳過** prescription/cram meta → device-local 不變量）。Homepage：常駐 header「考前救急」入口（不受 pressure gate）+ active 科卡片變身 rescue chip + 特訓吸收。**MVP 零 schema、全 device-local**（`localStorage` `neurons:rescue:v1`；不 bump Dexie / R2 SV / SYNCED_META_KEYS）。75 rescue Vitest + 1040 neurons 全綠 + typecheck clean + 瀏覽器 e2e 全程驗過。R2 同步為 follow-up `add-neurons-rescue-r2-sync`（已設計、SV 26→27、telemetry 維持 device-local）。詳見 `openspec/changes/add-neurons-single-subject-rescue/`. | 🔄 code-complete + verify green (2026-07-08)；剩 archive + commit |
| **M_3rd ext — 多科考前救急並存 (add-neurons-multi-subject-rescue)** | 把單科救急擴成**多科計劃並存**：每科各自考試日 / 每日分鐘 / 佇列 / 診斷 / 信心 / 止損，移除「一次一科」gate + 換科 confirm。UI 複數化：header → 救急總覽 list → 每科各自 scene（無中途切科 switcher）+ 每科 card 各自變 chip + cap 軟提示 3 / 硬上限 5 + setupExam 預設 **2026-07-17**。**Sync per-family**：`rescue:v1:plan:{familyId}` + conf/ovr 加 familyId 三段（68 conceptId 跨科共用 → 防碰撞）；**R2 SCHEMA_VERSION 27→28**（load-bearing 409 downgrade fence，防 stale v27 whole-snapshot push 抹掉 per-family keys）；零 Dexie bump（rides meta kv）。adoption 改 **brand-new-vs-existing SET**（全新帳號 carry over all anon / 有救急史帳號 drop anon-only + cloud-null 擋復活）；blitz/touch defer 改 **module-level** 跨 scene unmount；confidence 綁 run createdAt（activeRunRef freeze）；reload-toast **forward-only**（v27 bake 期無 toast）。設計 Opus×Codex×Fable 三方 + Codex 兩輪 propose review + Codex×Fable final 雙審 GO（修 5 項：matcher 三段 / confidence createdAt / blitz unmount / adoption malformed-cloud / doc）。typecheck 全綠 + **1120 vitest** + dev smoke（07/17）。詳見 `openspec/changes/archive/2026-07-09-add-neurons-multi-subject-rescue/`. | ✅ shipped (2026-07-09) — merged main + CF Pages deploy 綠；剩 owner prod 真機跨裝置 smoke |
