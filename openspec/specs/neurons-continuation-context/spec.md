# neurons-continuation-context Specification

## Purpose

Surface a 承上題 (continuation) question's preceding scenario in the neurons quiz UI, so
questions that refer back to an earlier scenario are answerable. Detection and chain
resolution use the shared `@study-rpg/core` helpers (`isContinuationQuestion` /
`resolvePrecedingChain`); the UI renders the resolved chain above the current question and
renders nothing for ordinary or unresolvable questions.
