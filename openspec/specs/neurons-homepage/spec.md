...
- **THEN** a single merged stat card renders as the homepage's top dashboard, directly above the maze centerpiece, with the ⚔️ 錯題出征 primary CTA as its top band and a causal-chain body (今日出征狀態 → 修復連線數據 → DMN 抽卡 進度)
- **AND** the top-to-bottom homepage order is stat-card dashboard → maze → study squad → family grid
