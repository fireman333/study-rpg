...
- **🆕 新題** — launches a quiz over ONLY the questions in that family the player has never answered (no `questionHistory` row for the question id). The chip SHALL show a badge with the unseen count and SHALL be disabled (with a 「全部答過」 affordance) when that count is 0.
- **🔄 錯題** — launches an SRS review quiz over that family's questions that are currently due (`nextDueAt <= now`). The chip SHALL show a badge with today's due count and SHALL be disabled (with a 「今日無到期」 affordance) when that count is 0.
...
- **THEN** its 🆕 新題 chip SHALL be disabled with a 「全部答過」 affordance
