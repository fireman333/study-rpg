...
二階 hospital home 的 pixel art 場景視覺系統 — tier-based scene asset、subject-bound doctor slot rendering、building click → upgrade modal。場景隨 `hospital.tier` 切換（診所 / 區域醫院 / 醫學中心 3 個 768×384 PNG），assigned doctors 依 SUBJECT_TO_ROOM mapping 在對應 room slot 顯示。提供 `?scene=off` URL query 作 emergency fallback。Lives in `apps/medexam2-hospital-tw`。
