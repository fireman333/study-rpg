...
熄燈儀式（「今天到此為止」）——低刺激的每日收工儀式，玩家可在任何時間主動觸發：connectome 轉暖色夜景、當日碰過的家族質性微光、睡眠固化 reframe（OE-anchored、僅一般機制）；之後首頁進入收工態（安靜/隱藏 push CTA，但非 hard lock，可「還是想再讀一下」還原）。純質性、不顯示題數/倒數/pass-fail。與今日處方箋成「一開一關」。純 local-only、零 schema。
