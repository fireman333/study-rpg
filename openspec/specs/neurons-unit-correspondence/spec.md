# neurons-unit-correspondence

## Purpose

Cross-links the three neurons-tw exam-prep systems — 考前講義 (handout), 考前救急 (rescue war map), 考前猜題 (cram 速看 + 押題) — at canonical `leafId` granularity so a learner can move along one unit between teaching, diagnosis, prediction, and practice. Handout teaching topics gain multi-value `data-leaf-ids` sub-anchors (region structure unchanged) validated by an independent build leaf-anchor gate; `HandoutPage` consumes `?leaf=` via a two-segment (subject, leaf) resolver (primary anchor → region fallback → inline unavailable + escape hatch, never region 0). Rescue chips deep-link at leaf granularity, cram 押題 link per-leaf and 速看 at subject level, and handout topics reverse-link to their 押題. Pure navigation + read-side resolution: no new persistent state (R2 `SCHEMA_VERSION` stays 28, no Dexie bump, no `SYNCED_META_KEYS` diff).
