# affinity-specialty-bonus Specification

## Purpose

Tier-based specialty multiplier applied to `affinity.correctCount` in the hospital (二階) track's quiz flow, mirroring the mastery treatment defined by `hospital-specialty-bonus`. When the bound partner doctor's `subjectId` exactly matches the active quiz subject, the affinity increment is multiplied by the same `SPECIALTY_MATCH_MULTIPLIER` table (P1=1.5 / P5=1.05). Cross-subject and no-partner correct answers retain the pre-existing `+1.0` behavior; wrong answers never touch the affinity table. Storage is float in-place (no Dexie migration); recruitment thresholds are unchanged; the RecruitmentBanner displays affinity with smart decimal precision (integer values stay integer, fractional values round to one decimal). The capability deliberately reuses the existing `getSpecialtyMultiplier` helper rather than introducing a parallel affinity-specific table — single source of truth for dogfood tuning.
