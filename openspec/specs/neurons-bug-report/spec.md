# neurons-bug-report Specification

## Purpose

In-app structured bug / suggestion reporting for `apps/neurons-tw`, mirroring the 二階 M4.5 pattern (documented in `docs/BUG_REPORTING.md`). The HelpMenu「回報問題」section opens a force-signed-in `BugReportModal` form, and `QuizModal` exposes an inline 🐞 entry scoped to the displayed question. Submissions auto-attach an opt-out context snapshot (route / neurons Dexie `game_state` / console-error ring buffer / sync diagnostics) and land in the shared Supabase `bug_reports` table with `app = 'neurons-tw'`, protected by the existing RLS (`auth.uid() = user_id`). neurons uses its own `NEURONS_BUG_REPORT_CATEGORIES` (12, a separate const from medexam's) and an additive migration (`0017`) that extends the shared `app` + `category` CHECK constraints. No IndexedDB / R2 schema change. Created by archiving change `add-neurons-bug-report`.
