...
The initial in-scope id set SHALL be the pilot booklets 112-1, 112-2, 113-1, 113-2, 114-1, 114-2. The contract SHALL be extensible across batches: questions added in a later batch (the full-scale follow-up, or the under-scanned 104-105 booklets once their parser lands) SHALL be governed by the identical contract without changing it, and earlier-shipped figures SHALL be unaffected by later batches.

#### Scenario: Pilot booklets are the initial in-scope set

- **WHEN** the pilot ships
- **THEN** recovered figures SHALL cover the 112-114 booklet questions in the in-scope id list, and no question outside the in-scope set SHALL gain a figure

#### Scenario: A later batch is attached under the same contract

- **WHEN** a later batch of figure-questions is added
- **THEN** it SHALL be attached under the identical contract (rasterized provenance-recorded crop, lazy-loaded, question text unchanged) and the pilot's already-shipped figures SHALL be unaffected
