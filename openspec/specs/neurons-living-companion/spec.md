...
The companion marcher SHALL resolve its artwork with a fallback so a future animated-frame asset can be swapped in without code change. Until the animated frames ship, the companion SHALL display the existing static `equipment:<id>` sprite within the band.

#### Scenario: missing animated asset falls back to static

- **WHEN** a dedicated animated companion asset (`companion:<id>`) is not registered
- **THEN** the band SHALL render the existing static `equipment:<id>` sprite (never a broken image)
