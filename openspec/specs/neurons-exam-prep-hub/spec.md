# neurons-exam-prep-hub

## Purpose

考前中心 hub：把分散的四個「考前」surface（考前猜題 `/cram`、考前講義 `/cram/handout`、五分鐘速看 `/cram/5min`、考前救急 RescueScene overlay）整合成單一 subject-led hub。題庫 group subtab 從 3 收成 2（題庫 + 考前中心）；`/cram` 首屏由上到下＝救急狀態條 → 11 科目卡（各帶講義/猜題 mini，選卡展開該科既有猜題 panel）→ 五分鐘速看一級卡 → PDF 沉底。救急在 hub 內「狀態條 + 可點就地進場」（開同一 RescueScene / 同一 global plan），並在 cram 押題 item 提供 leaf-level context 工具列（看講義｜對應練題｜救急狀態），handout topic 端沿用既有「本單元猜題」reverse-link 當 gateway。純導覽 / IA 重排 + 既有 deep-link 消費：零 schema、零 sync（R2 `SCHEMA_VERSION` 維持 28、無 Dexie bump、無 `SYNCED_META_KEYS` diff），保留全部 unit-correspondence deep-link。
