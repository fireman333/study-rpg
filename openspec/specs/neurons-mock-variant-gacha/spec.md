# neurons-mock-variant-gacha

## Purpose

A mock-exam-exclusive collectible neuron-variant line for neurons-mode. Submitting a
模擬考試 (full-paper closed-book) run rolls one variant from an INDEPENDENT, score-weighted
gacha pool (higher national-equivalent score → higher chance of a rarer tier), gated by a
per-paper daily cap + soft-pity. Deliberately separate from the maze taxonomy collection
(`neuron-variant-gacha` / `neuronVariants`), the DMN fate-card axis, and the maze energy
economy — it does not touch the "迷宮 = 唯一收集管道" invariant. Collected variants persist in a
dedicated synced `mockExamVariants` Dexie table; the roll bookkeeping (pity + daily-cap dates)
is device-local. Not counted toward the public leaderboard.
