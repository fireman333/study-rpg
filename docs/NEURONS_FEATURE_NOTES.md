...
## Multi-subject rescue (多科考前救急並存, M_3rd ext, 2026-07-09)

Extends single-subject rescue to **multiple coexisting per-family plans** + **cross-device per-family R2 sync** (SV 27→28 — supersedes the device-local MVP above; single-subject plan/confidence/override first went to R2 in `add-neurons-rescue-r2-sync` at SV 26→27, then this change went per-family at SV 27→28). Change: `openspec/changes/archive/2026-07-09-add-neurons-multi-subject-rescue/`.

- **Per-family sync keys** ⚠️ (**動 sync 前必讀**): `rescue:v1:plan:{familyId}` (was the single `rescue:v1:plan`); conf/ovr gain a familyId segment → `rescue:v1:{conf,ovr}:{planCreatedAt}:{familyId}:{id}` (68 conceptIds are shared across subjects → ovr would collide without the segment). `isSyncedRescueKey` admits the per-family plan prefix, **rejects the legacy single `rescue:v1:plan` AND v27-shaped 2-segment conf/ovr** (requires ≥3 segments with a non-empty familyId). **R2 `SCHEMA_VERSION` 27→28** — load-bearing (presign 409 downgrade fence stops a stale v27 whole-snapshot push from erasing the per-family keys), NOT merely reader-tolerance. NO Dexie bump.
- **Legacy migration is dual-path**: device-local (hydrate reads `db.meta` legacy `rescue:v1:plan` → per-family, then deletes the legacy row) + **cloud (backfill reads the legacy key from the RAW pulled bundle meta, bypassing the matcher which rejects it)** — both complete **before the first push**; a legacy `plan:null` (no familyId) is discarded, never used to clear per-family plans.
- **Adoption (anon→authed) = brand-new-vs-existing SET**: cloud bundle has NO rescue plan key → brand-new account → all anon plans carry over; cloud has ANY rescue plan key → existing-rescue → per-family SET (cloud-active verbatim wins / cloud explicit-null blocks anon resurrection / **anon-only family dropped**; `cloudFamilies.add` is INSIDE the valid-envelope check so a malformed cloud value can't shield an anon-only plan).
- **Lifecycle defer**: `startupSyncPending` gates ALL per-family envelope writes incl **markBlitzDone**; blitz/touch completion **DEFERS (not drops)** via module-level state in `rescue-store` (`deferBlitzDone` / `deferTouchLastStudied` / `flushPendingRescueLifecycle`), flushed by `useSync`'s startup-settle `.then` (survives the scene unmounting) + the scene effect; `resolvePendingBlitzFlush` (`rescue-blitz-defer.ts`) guards replaced/abandoned runs (createdAt must match). DON'T revert to a component-ref defer (loses the write on unmount).
- **Confidence bound to run**: `recordConfidence(familyId, planCreatedAt, qid, signal)` takes explicit createdAt and no-ops on mismatch/abandoned; RescueScene freezes `{familyId, createdAt}` into `activeRunRef` at answer-phase entry; QuizModal reads the scene-injected `onRescueConfidence` (NOT the singleton `getActivePlan`) — prevents a sync-replace mid-answer writing to the new run.
- **Reload toast is forward-only**: `SyncReloadToast` / `sync-reload-signal.ts` ship IN v28, so live v27 tabs during the v27→v28 bake get NO toast (held only by the presign rate-limiter until reload); the toast activates from v28→v29+.
- **UI**: header 考前救急 → overview list → per-family scene (no mid-session switcher); each active-plan family card renders its own chip; cap soft-3 / hard-5 (UI-level — two devices at 4 can converge to 6, store/backfill handle it); setupExam default 2026-07-17.
- Tests: `apps/neurons-tw/src/__tests__/rescue-{sync,store,blitz-defer,conflict-recovery,push-coalescing}.test.ts` (**1120 neurons tests green**). Review: Opus×Codex×Fable design panel → Codex 2 propose-review rounds → Codex final NO-GO (3 blocking) + Fable GO (caught the toast-forward-only gap) → 5 fixes → Codex re-review GO + Fable GO. **Codex sandbox tip**: `codex exec` was EPERM-blocked writing `~/.codex/sessions` under the Bash read-only sandbox — run via Bash `dangerouslyDisableSandbox`, NOT sudo chown.

Full change reference: `openspec/changes/archive/2026-07-09-add-neurons-multi-subject-rescue/`.
